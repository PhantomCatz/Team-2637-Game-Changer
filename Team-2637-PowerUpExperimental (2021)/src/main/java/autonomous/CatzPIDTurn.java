...
public class CatzPIDTurn {
	 ...
	 ***************************************************************************/
	public static void PIDturn(double degreesToTurn, double timeoutSeconds, double maxSpeed) {
		...
		while (done == false) {
			...
			if (Math.abs(currentError) < PID_TURN_THRESHOLD) {
				...
				if (functionTimer.get() > timeoutSeconds) {
					 ...
					 *******************************************************************/
					CatzRobotMap.drive.tankDrive(Math.min(power, maxSpeed), -Math.min(power, maxSpeed));
